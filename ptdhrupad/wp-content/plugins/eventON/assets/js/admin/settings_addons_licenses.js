...
jQuery(document).ready(function($){
	...
	// load forms for lightbox
		$('#evo_license_form_trig').on('click',function(){
			var ajaxdataa = {};
	...
	// Activate subscription
		$('.evo_subscription_form_trigger').on('click',function(){
			var ajaxdataa = {};
			OBJ = $(this);
			ajaxdataa['action'] = 'eventon_admin_get_views';
			ajaxdataa['type'] = 'evo_subscription_activation_form';

			$.ajax({
				beforeSend: function(){
					$('.ajde_lightbox_title').html('Activate EventON Auto Updates Subscription');
				},
				type: 'POST',dataType:'json',url:the_ajax_script.ajaxurl,data: ajaxdataa,
				success:function(data){	
					if(data.status=='good'){
		...
		$('body').on('click','.eventon_activate_subsctiption_btn',function(){
			BTN = $(this);
			FORM = BTN.closest('.evo_license_section_form');
			var ajaxdataa = {};

			// field validation
				errors = 0;
				FORM.find('input.fields').each(function(){
					if($(this).val()==''){
					 	errors++;
					}else{
			...
			// activate subscription				
				ajaxdataa['action'] = 'eventon_activate_subscription';
				$.ajax({
		...
		// deactivate subscription
			$('body').on('click','.evo_deactivate_sub',function(){
				BTN = $(this);
				var data_arg = {
					action:'eventon_deactivate_subscription',
				};
				var addon = BTN.closest('.evo_auto_update_subscription');
				$.ajax({
					...
					dataType:'json',
					success:function(data){
						if(data.html !='')	addon.html( data.html );
						alert( data.msg);	
