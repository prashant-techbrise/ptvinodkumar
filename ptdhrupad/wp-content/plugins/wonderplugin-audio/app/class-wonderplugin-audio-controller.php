...
class WonderPlugin_Audio_Controller {
	...
	function generate_body_code($id, $content, $attributes, $has_wrapper) {
		...
		return $this->model->generate_body_code($id, $content, $attributes, $has_wrapper);
	}
	...
	function radionomy_gettracklist($post) {
		...
		return $this->model->radionomy_gettracklist($post);
	}
	
	function shoutcast_gettracklist($post) {
		
		return $this->model->shoutcast_gettracklist($post);
	}
	
	function shoutcast_station_gettracklist($post) {
	
		return $this->model->shoutcast_station_gettracklist($post);
	}
}